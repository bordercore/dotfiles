...
- [[#early-initialization-and-performance][Early Initialization & Performance]]
- [[#package-management][Package Management]]
  - [[#sources][Sources]]
  - [[#setup][Setup]]
- [[#core-settings][Core Settings]]
- [[#user-interface][User Interface]]
- [[#editing-and-text-manipulation][Editing & Text Manipulation]]
- [[#key-bindings][Key Bindings]]
- [[#buffers-and-sessions][Buffers & Sessions]]
  - [[#backups][Backups]]
  - [[#buffer-management][Buffer Management]]
  - [[#session-management][Session Management]]
- [[#navigation-and-completions][Navigation & Completions]]
  ...
  - [[#treemacs][Treemacs]]
  - [[#dired][Dired]]
- [[#help-and-documentation][Help & Documentation]]
- [[#org-mode][Org Mode]]
...
- [[#ai-tools][AI Tools]]
  - [[#chatgpt][ChatGPT]]
  - [[#codeium][Codeium]]
- [[#external-tools-and-integration][External Tools & Integration]]
  - [[#games][Games]]
- [[#wrapup][Wrapup]]
...
Reinstall rainbow-delimiters if autoloads file is missing
#+BEGIN_SRC emacs-lisp
(let ((elpa-dir (expand-file-name "elpa" package-user-dir))
      (autoloads-file nil))
  (when (file-directory-p elpa-dir)
    (dolist (dir (directory-files elpa-dir t "^rainbow-delimiters"))
      (let ((file (expand-file-name "rainbow-delimiters-autoloads.el" dir)))
        (when (file-exists-p file)
          (setq autoloads-file file)))))
  (when (not autoloads-file)
  ...
  (let ((theme-name (getenv "EMACS_THEME")))
    (if theme-name
        (condition-case err
          ...
          (error (message "Error loading theme %s: %s" theme-name (error-message-string err))))
...
(dolist (file '("~/.emacs.d/themes/current-theme-custom.el"))
  (when (file-exists-p file)
    (condition-case err
        (load-file file)
      (error (message "Error loading theme customization file %s: %s" file (error-message-string err))))))

;; On macOS, ensure Menlo font is set after theme loads
(when (equal system-type 'darwin)
  ;; Set font for all frames (including new ones)
...
(use-package company
  ...
  (setq-default
   ...
   ;; Display a drop down with preview
...
(setq org-capture-templates
      (list
       (list "c" "Contact" 'entry
             (list 'file+headline "~/docs/personal/contacts.org" "Contacts")
             my/org-contacts-template
             :empty-lines 0)
       (list "t" "Todo" 'entry
             (list 'file+headline org-default-notes-file "Tasks")
             "* TODO %?  %t  %^g")))
#+END_SRC
...
#+begin_src emacs-lisp
(advice-add 'org-todo :after #'mrb/insert-created-timestamp)
#+end_src
...
(let ((existing (getenv "PYTHONPATH"))
      (bordercore-root (expand-file-name "~/dev/django/bordercore"))
      (bordercore-core (expand-file-name "~/dev/django/bordercore/bordercore")))
  (when (file-directory-p bordercore-root)
    (setenv "PYTHONPATH"
            (mapconcat 'identity
                       (delete-dups (append (split-string (or existing "") ":")
                                            (list bordercore-root bordercore-core)))
                       ":"))))
#+end_src
...
#+begin_src emacs-lisp
(defvar bc/trusted-project-paths nil
  "List of directory paths that are trusted for loading directory-local variables.
Paths can be added via customize-variable or in local.el")
...
#+BEGIN_SRC emacs-lisp
(defun magit-fullscreen-wrapper (orig-fun &rest args)
  (window-configuration-to-register :magit-fullscreen)
  (apply orig-fun args)
  (delete-other-windows))

(advice-add 'magit-status :around #'magit-fullscreen-wrapper)
#+END_SRC
...
(with-eval-after-load 'chatgpt-shell
  ...
  (defun chatgpt-shell-proofread-or-improve--impl ()
    "Internal implementation: Prompt to either spell-check or improve clarity on region or paragraph."
    (interactive)
    (let ((choice
           (ivy-completing-read
      ...
      (pcase choice
        ...
        (_ (user-error "Invalid choice: %s" choice)))))  ; Close pcase, let, defun
  )  ; Close with-eval-after-load

(defun chatgpt-shell-proofread-or-improve ()
  "Prompt to either spell-check or improve clarity on region or paragraph.
This function ensures chatgpt-shell is loaded before calling the actual implementation."
  (interactive)
  (require 'chatgpt-shell)
  (chatgpt-shell-proofread-or-improve--impl))
