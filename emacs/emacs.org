...
(global-set-key (quote [f4]) 'projectile-ag)
  ...
  (setenv "PYTHONPATH"
          (mapconcat 'identity
                     (delete-dups (append (split-string (or existing "") ":")
                                          '("/home/jerrell/dev/django/bordercore_project" "/home/jerrell/dev/django/bordercore_project/bordercore")))
                     ":")))
#+end_src
...
(use-package flycheck
  ...
  ;; Use my own version of Python rather than the one installed by the system
  (setq flycheck-python-flake8-executable "/usr/bin/python3.8")
...
(use-package chatgpt-shell
  ...
  (setq chatgpt-shell-model-versions '("gpt-4.1"))
  )
#+end_src
...
(defun run-gpt-commit ()
  "Run gpt-commit.py from the git repo root, pulling OPENAI_API_KEY from ~/.netrc."
  (interactive)
  (let ((git-root (magit-toplevel)))
    (if (not git-root)
      ...
      (let* ((default-directory git-root)
             ...
             (api-key       nil))
        ;; Parse ~/.netrc for "machine api.openai.com" stanza
        (when (and (file-readable-p netrc-file)
                   (with-temp-buffer
                     (insert-file-contents netrc-file)
                     (goto-char (point-min))
                     ;; find the machine line...
                     (re-search-forward "^machine[ \t]+api\\.openai\\.com\\b" nil t)
                     ;; ...then the next password line
                     (re-search-forward "^\\s-*password[ \t]+\\(.+\\)$" nil t)))
          (setq api-key (match-string 1)))
        (when api-key
          (setenv "OPENAI_API_KEY" api-key))
        (condition-case err
            (let ((output (shell-command-to-string
                           "python3 /home/jerrell/dev/gpt-commit.py")))
              (if (string-empty-p (string-trim output))
                  (message "gpt-commit.py produced no output")
                (insert output)))
          (error (message "Error running gpt-commit.py: %s"
